...
import { cannyApiRequest, cannyApiRequestAllItemsSkip } from './GenericFunctions';
...
import { portalCommentOperations, portalCommentFields } from './descriptions/PortalCommentDescription';
			...
			try {
				...
				if (resource === 'post') {
					if (operation === 'create') {
						...
						responseData = await cannyApiRequest.call(this, 'POST', '/posts/create', {
							boardID,
							title,
							details,
							authorID,
						...
						responseData = await cannyApiRequest.call(this, 'POST', '/posts/retrieve', {
							id: postID,
						});
